...
def submit():
    ...
    r=requests.post("http://127.0.0.1:5000/read",data=data_dict)
    json_r=r.json()
    print(json_r)
    print(json_r['data'])
    ...
    treeview = ttk.Treeview(table_window, columns=('logtime', 'power_supply', 'readback'))
    treeview.heading('logtime', text='Logtime')
    treeview.heading('power_supply', text='Power Supply')
    treeview.heading('readback', text='Readback Value')
    treeview.pack()

    for item in json_r['data']:
        logtime = str(item['Logtime'])
        power_supply = str(item['Power Supply Number'])
        readback = str(item['Readback Value']) 
        treeview.insert('', 'end', values=(logtime, power_supply, readback))
